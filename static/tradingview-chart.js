...
// เพิ่มตัวแปร global สำหรับการตรวจจับแนวรับ
let supportSignals = [];
let supportMarkersEnabled = true;
let swingPoints = [];

// ฟังก์ชันตรวจจับสัญญาณแนวรับ
function detectSupportSignals() {
    if (!supportMarkersEnabled || data.length < 3) return;
    
    // ลบสัญลักษณ์แนวรับเดิมทั้งหมด
    //clearAllSupportMarkers();
    // ตรวจสอบแต่ละแท่งเทียนสำหรับสัญญาณแนวรับ
    for (let i = 2; i < data.length; i++) {
        const current = data[i];
        const prev1 = data[i-1];
        const prev2 = data[i-2];
        
        // ตรวจสอบรูปแบบแท่งเทียนกลับตัวขึ้น (Bullish Reversal Patterns)
        if (isHammer(current) ||
            isBullishEngulfing(prev1, current) ||
            isMorningStar(prev2, prev1, current) ||
            isPiercingLine(prev1, current) ||
            isTweezerBottom(prev1, current)
            ) 
        {
            
            // ตรวจสอบเงื่อนไขเพิ่มเติมเพื่อยืนยันแนวรับ
            if (isAtSupportLevel(current)) {
                const reversalMarkers = addSupportMarker(current, 'Bullish Reversal');
                candleSeries.setMarkers(reversalMarkers);
            }
        }
    }
...
// ฟังก์ชันเพิ่มสัญลักษณ์แนวรับบนกราฟ
function addSupportMarker(candle, patternName) {
    // ตรวจสอบว่าไม่มีการทำเครื่องหมายที่แท่งนี้แล้ว
    if (supportSignals.some(s => s.time === candle.time)) return;
    
    // บันทึกลงใน supportSignals
    supportSignals.push({
        time: candle.time,
        position: 'belowBar',
        color: '#00C805',
...
// แก้ไขฟังก์ชัน clearAllSupportMarkers
function clearAllSupportMarkers() {
    supportSignals.forEach(signal => {
        try {
            // ตรวจสอบว่า series ยังมีอยู่ในแผนภูมิหรือไม่ก่อนลบ
            if (signal.series && chart.series().includes(signal.series)) {
                chart.removeSeries(signal.series);
            }
        } catch (error) {
...
// ฟังก์ชันตรวจสอบรูปแบบ Hammer
function isHammer(candle) {
    const bodySize = Math.abs(candle.open - candle.close);
    const lowerShadow = candle.close > candle.open ? 
...
// ฟังก์ชันตรวจสอบรูปแบบ Bullish Engulfing
function isBullishEngulfing(prevCandle, currentCandle) {
    return prevCandle.close < prevCandle.open && // แท่งก่อนเป็นขาลง
           currentCandle.open < prevCandle.close && // เปิดต่ำกว่าปิดแท่งก่อน
           currentCandle.close > prevCandle.open; // ปิดสูงกว่าเปิดแท่งก่อน
}
// ฟังก์ชันตรวจสอบรูปแบบ Morning Star
function isMorningStar(prev2, prev1, current) {
    return prev2.close < prev2.open && // แท่งที่ 1 เป็นขาลง
           Math.abs(prev1.open - prev1.close) < (prev2.close - prev2.open) * 0.3 && // แท่งที่ 2 เป็น Doji หรือแท่งเล็ก
           current.close > current.open && // แท่งที่ 3 เป็นขาขึ้น
           current.close > (prev2.open + prev2.close) / 2; // ปิดเกินกลางแท่งที่ 1
}
// ฟังก์ชันตรวจสอบรูปแบบ Piercing Line
function isPiercingLine(prevCandle, currentCandle) {
    return prevCandle.close < prevCandle.open && // แท่งก่อนเป็นขาลง
           currentCandle.open < prevCandle.low && // เปิดต่ำกว่าต่ำสุดแท่งก่อน
           currentCandle.close > (prevCandle.open + prevCandle.close) / 2 && // ปิดเกินกลางแท่งก่อน
           currentCandle.close < prevCandle.open; // แต่ไม่เกินเปิดแท่งก่อน
}
// ฟังก์ชันตรวจสอบรูปแบบ Tweezer Bottom
function isTweezerBottom(prevCandle, currentCandle) {
    return Math.abs(prevCandle.low - currentCandle.low) <= (prevCandle.high - prevCandle.low) * 0.05 && // ต่ำสุดใกล้เคียงกัน
           prevCandle.close < prevCandle.open && // แท่งก่อนเป็นขาลง
           currentCandle.close > currentCandle.open; // แท่งปัจจุบันเป็นขาขึ้น
}

// ฟังก์ชันตรวจสอบว่าอยู่ในแนวรับหรือไม่
function isAtSupportLevel(candle) {
    // วิธีที่ 2: ตรวจสอบกับจุด Swing Low ก่อนหน้า
    const swingLows = swingPoints.filter(p => p.type === 'low');
    for (const swingLow of swingLows) {
        if (Math.abs(candle.low - swingLow.price) <= swingLow.price * 0.01) {
            return true;
        }
    }
    
    // วิธีที่ 3: ตรวจสอบกับค่าเฉลี่ยเคลื่อนที่
    const ema5 = candle.ema20;
    if (Math.abs(candle.low - ema5) <= ema5 * 0.01) {
        return true;
    }
